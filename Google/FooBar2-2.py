...
# her minions on their toes. But you've noticed a flaw in the algorithm - it eventually loops back on itself, 
# so that instead of assigning new minions as it iterates, it gets stuck in a cycle of values 
# so that the same minions end up doing the same tasks over and over again. You think proving this to Commander 
...
# the algorithm will reach the cycle of values [210111, 122221, 102212] and it will stay in this cycle no 
# matter how many times it continues iterating. Starting with n = 1211, the routine will reach the integer 6174, 
...
# where 2 <= k <= 9 and 2 <= b <= 10, write a function solution(n, b) which returns the length of the 
# ending cycle of the algorithm above starting with n. For instance, in the example above, solution(210022, 3) 
# would return 3, since iterating on 102212 would return to 210111 when done in base 3. 
